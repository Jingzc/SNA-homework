    ...
    def calc_user_sim(self):
       ...
       # usercommunity={}
        ...
        for user,friend in self.socialset.items():
            for u in friend:
                usersimf_mat.setdefault(u,defaultdict(int))
                for f in friend:
                    if u==f:
                        continue
                    #usersimf_mat[u][f]+=1
                    usersimf_mat[u][f]+=1/math.log(1+len(friend)) 
        ...
        for u, related_users in usersim_mat.items():
            for v, count in related_users.items():
                ...
                elif v not in usersimf_mat[u].keys():
                    usersim_mat[u][v] = (1-a)*count / math.sqrt(len(self.trainset[u]) * len(self.trainset[v]))
...
if __name__ == '__main__':
    ...
    trustfile=os.path.join(r'data\trust.csv')
    usercf = UserBasedCF()
    usercf.generate_dataset(ratingfile)
    usercf.social_network(trustfile)
